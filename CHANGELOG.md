...
- A graph walk could seal with no Results section when the AI gateway rate-limited it near the end. The paper agent's call runs on a thread the pipeline has to wait for, and a 429 asking for an 84 s wait held the pipeline 54 s past the Writers' deadline, which used up the time kept for the Narrator. The paper agent, the sense check and the Narrator now give up by their stage's deadline, the pipeline no longer waits for a thread still running when its stage ends, and the page says when a walk ran out of time, including when the Narrator itself gave up at the deadline, instead of saying its Results section failed its checks.
- A walk that kept fewer statements than planned had its Results section refused for being short: a Rap1 walk kept 6 statements, the Narrator retold them in 368 words, and the plan's 400-word floor left the page with none. The floor is now 50 words per kept statement, up to the plan's.
- A walk's statements and Results could still narrate the walk in other words ("a network walk from the seed miRNA miR-151-3p", "the seed miRNA mmu-miR-3074-1-3p"), which the check for "cluster" and "the walk" let through. Those forms are refused too; a miRNA's seed region, a plant's seed miRNAs and a kinesin walking along a microtubule are not.
