...
- The container deployment recycled its uWSGI worker in the middle of a job. There is a single worker, and `reload-on-rss` was set to 4096 MB while one large multi-omic job already resides at about 2 GB and the in-process job cache holds recently-touched jobs beside it, so a legitimate working set crossed the ceiling and the worker was recycled mid-run, killing the job. On the production host, which has tens of GB of RAM (mongod is capped separately with `--wiredTigerCacheSizeGB`), the limit is now 24 GB: high enough that real work never trips it, low enough to still catch a runaway before the out-of-memory killer does.
