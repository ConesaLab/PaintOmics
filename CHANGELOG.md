...
- The container deployment recycled its uWSGI worker in the middle of a job. There is a single worker, and `reload-on-rss` was set to 4096 MB while one large multi-omic job already resides at about 2 GB and the in-process job cache holds recently-touched jobs beside it, so a legitimate working set crossed the ceiling and the worker was recycled mid-run, killing the job. The limit is now 12 GB: several times the working set, yet still below the memory left after mongod's `--wiredTigerCacheSizeGB` and the OS, so uWSGI's graceful reload fires before the kernel out-of-memory killer would.
