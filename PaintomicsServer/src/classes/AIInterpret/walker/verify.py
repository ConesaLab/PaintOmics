...
# Syk kinase seed via a jump", "a network walk from the seed miRNA". Name the
# genes instead. Only the unambiguous forms are refused: "seed" is ordinary in
# a plant job (seed storage proteins) and in a miRNA's seed region, and "a jump
# at 12h" describes values, so bare "seed" and "jump" are left to the prompts.
JARGON_RE = re.compile(r"\b(clusters?|(?:a|an|the|this|our|each) (?:network |graph |pathway )?walk(?:er)?s?"
                       r"|walk(?:ed|ing|s)? (?:from|to|through|along|across)|via a jump"
                       r"|jump(?:s|ed|ing)? (?:to|from|back)|seed (?:nodes?|mirnas?))\b", re.I)
